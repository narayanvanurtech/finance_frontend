...
import { Task, UpdateTaskPayload } from "@/api/taskApi";
...
const STAGES = [
  ...
  },
  {
    ...
    icon: <DeferredIcon className="h-4 w-4 text-purple-700" />,
  },
  {
    id: "in_progress",
    name: "In Progress",
...
const TasksPipeline = () => {
  ...
  const filteredTasks = tasks.filter((task: Task) => {
    ...
    const matchesFilter =
      ...
      (activeFilter === "due_soon" &&
        task.createdAt &&
        new Date(task.createdAt).getTime() - Date.now() <
          3 * 24 * 60 * 60 * 1000) ||
  ...
  const tasksByStage = STAGES.reduce((acc, stage) => {
    // Create a reverse mapping from stage ID to status
    ...
    acc[stage.id] = filteredTasks.filter((task: Task) => {
      return task.status === stageToStatusMap[stage.id];
    });
    return acc;
  ...
  const handleDrop = async (e: React.DragEvent, targetStageId: string) => {
    ...
    const stageToStatusMap: Record<
      string,
      "open" | "progress" | "done" | "cancel"
    > = {
      not_started: "open",
    ...
    try {
      // Get the new status directly from the map
      const newStatus = stageToStatusMap[targetStageId];
      if (newStatus && draggedTask.id) {
        // Create a task update object with only the status change
        const taskUpdate: UpdateTaskPayload = {
          status: newStatus,
        };

        // Call updateTask with taskId and update payload
  ...
  const handleTaskClick = (task: Task) => {
    setSelectedTasks((prev: string[]) => {
      ...
      if (isSelected) {
        setSelectedItems(prev.filter((id: string) => id !== task.id));
        return prev.filter((id: string) => id !== task.id);
      } else {
        ...
        <div className="flex items-center justify-end">
          <div className="relative w-80">
            ...
            <input
              ...
              className="block w-full pl-10 pr-4 py-2.5 border border-gray-300 rounded-lg bg-gray-50 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent"
              value={searchTerm}
