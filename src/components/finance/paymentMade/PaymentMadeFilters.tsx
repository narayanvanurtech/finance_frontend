...
const PaymentMadeFilters: React.FC<PaymentMadeFiltersProps> = ({
  onSearch,
  onClear,
  loading,
}) => {
  ...
  const handleFilterChange = (
    ...
    if (!value || value === "" || value === "all" || value === "default") {
      ...
      if (key === "sortOrder" && (value === "asc" || value === "desc")) {
        ...
        key === "paymentType" &&
        (value === "Payment" || value === "Advance")
      ) {
        newFilters[key] = value;
      } else if (key !== "sortOrder" && key !== "paymentType") {
  ...
  const hasActiveFilters = Object.entries(filters).some(([key, value]) => {
    ...
    if (key === "paymentType" && value === "all") return false;
    if (key === "paymentMethod" && value === "all") return false;
      ...
      <div className="flex flex-col space-y-4">
        <div className="flex flex-wrap items-center gap-4">
          <div className="relative flex-1 min-w-[200px]">
            ...
            <Input
              placeholder="Search payments by number, vendor, or purchase..."
              className="pl-10"
              ...
              <span className="bg-blue-500 text-white text-xs px-2 py-1 rounded-full">
                {
                  Object.entries(filters).filter(([key, value]) => {
                    if (!value || value.length === 0) return false;
                    if (key === "paymentType" && value === "all") return false;
                    if (key === "paymentMethod" && value === "all")
                      return false;
                    if (key === "sortBy" && value === "default") return false;
                    if (key === "sortOrder" && value === "desc") return false;
                    return true;
              ...
              <div>
                ...
                >
                  ...
                  <SelectContent>
                    ...
                    <SelectItem value="receiptDate">Date</SelectItem>
                    <SelectItem value="totalAmount">Amount</SelectItem>
                    <SelectItem value="receiptNo">Receipt Number</SelectItem>
                  </SelectContent>
                </Select>
              </div>
...
export default PaymentMadeFilters;
