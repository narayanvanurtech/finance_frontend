...
import { Button, Select, MenuItem, Menu } from "@mui/material";
...
export default function SubNav({
  ...
  const { deleteAllMeetings, bulkDeleteMeetings, fetchMeetings } =
    useMeetingsStore();
  const { bulkDeleteCalls, fetchAllCalls } = useCallsStore();
  const { bulkDeleteTasks, fetchTasks } = useTasksStore();
  ...
  const handleExportTasks = async () => {
    ...
    try {
      ...
      // Export tasks - Feature disabled as exportTasks function doesn't exist in store
  ...
  const handleBulkDelete = async () => {
    ...
    try {
      switch (currentSection) {
        case "leads":
          if (selectedItems.length > 0 && user?.companyId) {
      ...
      case "Send Bulk Mail":
        if (selectedItems.length === 1) {
          ...
          try {
            ...
            const response = await getAllLeads(user.companyId);
            const allLeads = response.data.response;
            const selectedLead = allLeads.find(
              (lead: any) => lead.id === selectedItems[0]
            );
      ...
      <div className="px-6 py-4">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 flex-shrink-0 mt-4">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center gap-2">
            {/* SearchBar component not imported - placeholder */}
          </div>
